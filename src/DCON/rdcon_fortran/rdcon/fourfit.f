...
c-----------------------------------------------------------------------
      ...
      TYPE(cspline_type),TARGET :: fmats,gmats,kmats,
     $                             fmats_gal,gmatsp,kmatsp
      TYPE(spline_type) :: k0s
      INTEGER, DIMENSION(:), POINTER :: ipiva
      COMPLEX(r8), DIMENSION(:,:), POINTER :: amat,bmat,cmat,cmatp
...
c-----------------------------------------------------------------------
     ...
     $     fmat,gmat,hmat,kmat,temp1,temp2,hmatp,kmatp,gmatp
      TYPE(cspline_type) :: amats,bmats,cmats,cmatsp
...
c-----------------------------------------------------------------------
     ...
     $     ipiva(mpert),jmat(-mband:mband),cmatp(mpert,mpert))
      ...
      CALL cspline_alloc(fmats_gal,mpsi,(mband+1)*(2*mpert-mband)/2)
      CALL cspline_alloc(gmatsp,mpsi,(mband+1)*(2*mpert-mband)/2)
      CALL cspline_alloc(kmatsp,mpsi,(2*mband+1)*mpert)
      fmats%xs=rzphi%xs
      gmats%xs=rzphi%xs
      kmats%xs=rzphi%xs
      fmats_gal%xs=rzphi%xs
      gmatsp%xs=rzphi%xs
      kmatsp%xs=rzphi%xs
      fmats%fs=0
      gmats%fs=0
...
c-----------------------------------------------------------------------
      IF(sas_flag)THEN
         ...
         CALL cspline_alloc(cmatsp,mpsi,mpert**2)
         amats%xs=sq%xs
         bmats%xs=sq%xs
         cmats%xs=sq%xs
         cmatsp%xs=sq%xs
...
c-----------------------------------------------------------------------
      DO ipsi=0,mpsi
         ...
         pre=sq%fs(ipsi,2)
         p1=sq%fs1(ipsi,2)
c         CALL toolbox_preflatcof(psifac,dpsi_intvl,dpsi1_intvl,prefac,
c     $                           cof,cof1)
c         p1=p1*cof
         q=sq%fs(ipsi,4)
         q1=sq%fs1(ipsi,4)
...
c-----------------------------------------------------------------------
c     reduced pressure matrix for relax perturbed equilibrium.
c-----------------------------------------------------------------------
               kmatp(ipert,jpert)=twopi*ifac*chi1
     $                           *(g23(dm)+g33(dm)*m1/nn)
               cmatp(ipert,jpert)=twopi*ifac*(
     $              twopi*ifac*chi1*singfac2*(nn*g12(dm)+m1*g31(dm))
     $              -q1*chi1*(nn*g23(dm)+m1*g33(dm)))
     $              -twopi*ifac*(jtheta*singfac1*imat(dm)
     $              +nn*prefac*p1/chi1*jmat(dm))
               hmatp(ipert,jpert)=(q1*chi1)**2*g33(dm)
     $              +(twopi*chi1)**2*singfac1*singfac2*g11(dm)
     $              -twopi*ifac*chi1*dm*q1*chi1*g31(dm)
     $              +jtheta*q1*chi1*imat(dm)+prefac*p1*jmat1(dm)
            ENDDO
         ENDDO
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
         IF(sas_flag)THEN
            ...
            cmatsp%fs(ipsi,:)=RESHAPE(cmatp,(/mpert**2/))
         ENDIF
c-----------------------------------------------------------------------
c     factor A.
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
         ...
         temp2=cmatp
         CALL zhetrs('L',mpert,mpert,amat,mpert,ipiva,temp2,mpert,info)
         kmatp=emat-MATMUL(CONJG(TRANSPOSE(kmatp)),temp2)
         gmatp=hmatp-MATMUL(CONJG(TRANSPOSE(cmatp)),temp2)
c-----------------------------------------------------------------------
c     diagnose.
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
         ...
         ENDDO
c-----------------------------------------------------------------------
c     store Hermitian matrices F for galerkin method
c-----------------------------------------------------------------------
         iqty=1
         DO jpert=1,mpert
            DO ipert=jpert,MIN(mpert,jpert+mband)
               fmats_gal%fs(ipsi,iqty)=fmatb(1+ipert-jpert,jpert)
               iqty=iqty+1
            ENDDO
         ENDDO
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
         ...
         DO jpert=1,mpert
            DO ipert=jpert,MIN(mpert,jpert+mband)
               ...
               gmatsp%fs(ipsi,iqty)=gmatp(ipert,jpert)
               iqty=iqty+1
            ENDDO
         ENDDO
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
         ...
         DO jpert=1,mpert
            DO ipert=MAX(1,jpert-mband),MIN(mpert,jpert+mband)
               ...
               kmatsp%fs(ipsi,iqty)=kmatp(ipert,jpert)
               iqty=iqty+1
            ENDDO
         ENDDO
      ENDDO
...
c-----------------------------------------------------------------------
      ...
      CALL cspline_fit(fmats_gal,"extrap")
      CALL cspline_fit(kmatsp,"extrap")
      CALL cspline_fit(gmatsp,"extrap")
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      ...
      IF(bin_fmat)CALL fourfit_write_matrix(fmats_gal,"fmat_gal",.TRUE.)
