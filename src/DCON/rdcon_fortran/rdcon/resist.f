...
c-----------------------------------------------------------------------
      ...
      p=sq%f(2)
      p1=sq%f1(2)
c      CALL toolbox_preflatcof(psifac,dpsi_intvl,prefac,cof,cof1)
c      p1=p*cof1+p1*cof
      v1=sq%f(3)
      v2=sq%f1(3)
      q=sq%f(4)
...
c      sing%restype%e=p1*v1/(q1*chi1**2)**2*avg(1)
c     $     *(twopi*twopif*q1*chi1/avg(5)-v2)
c      sing%restype%f=(p1*v1/(q1*chi1**2))**2*(avg(1)*avg(3)
c     $     +(twopif/chi1/twopi)**2*(avg(1)*avg(4)-avg(2)**2))
c      sing%restype%h=twopif*p1*v1/(q1*chi1**3)*(avg(2)-avg(1)/avg(5))
...
c      sing%restype%taua=SQRT(sing%restype%rho*sing%restype%m)
c     $     /ABS(twopi*nn*sing%q1*chi1/v1)
c      sing%restype%taur=avg(1)/avg(5)/sing%restype%eta

      sing%restype%v1=v1/volume
c-----------------------------------------------------------------------
c     diagnose.
c-----------------------------------------------------------------------
      IF(diagnose)THEN
