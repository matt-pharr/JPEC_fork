...
!$OMP THREADPRIVATE(/DLS002/)
      ...
      SUBROUTINE LSODE2(F, NEQ, Y, T, TOUT, ITOL, RTOL, ATOL,
...
C              If only the derivative dy/dt is needed, use DINTDY2
C              instead.
C
...
C                               immediately.
C     CALL DSRCOM2(RSAV,ISAV,JOB)  Saves and restores the contents of the
...
C     CALL DINTDY2(,,,,,)        Provide derivatives of y, of various
C     (see below)               orders, at a specified point t, if
C                               desired.  It may be called only after a
C                               successful return from LSODE.  Detailed
C                               instructions follow.
C
C     Detailed instructions for using DINTDY2
C     --------------------------------------
C     The form of the CALL is:
C
C           CALL DINTDY2 (T, K, RWORK(21), NYH, DKY, IFLAG)
C
...
C     block, use subroutine DSRCOM2 (see Part 2 above).
C
C
...
C                 1983), pp. 55-64.
C***ROUTINES CALLED  DEWSET, DINTDY2, DUMACH, DSTODE2, DVNORM, XERRWD
C***COMMON BLOCKS    DLS002
...
C  DINTDY2   computes an interpolated value of the y vector at t = TOUT.
C  DSTODE2   is the core integrator, which does one step of the
C           integration and the associated error control.
C  DCFODE   sets all method coefficients and test constants.
C  DPREPJ2   computes and preprocesses the Jacobian matrix J = df/dy
C           and the Newton iteration matrix P = I - h*l0*J.
C  DSOLSY2   manages solution of linear system in chord iteration.
C  DEWSET   sets the error weight vector EWT before each step.
C  DVNORM   computes the weighted R.M.S. norm of a vector.
C  DSRCOM2   is a user-callable routine to save and restore
...
C then those local to Subroutine DSTODE2, and finally those used
C for communication.  The block is declared in subroutines
C LSODE, DINTDY2, DSTODE2, DPREPJ2, and DSOLSY2.  Groups of variables are
...
C If ISTATE = 3, set flag to signal parameter changes to DSTODE2. -------
      JSTART = -1
...
C-----------------------------------------------------------------------
 ...
 210  IF ((TN - TOUT)*H .LT. 0.0) GO TO 250
      CALL DINTDY2 (TOUT, 0, RWORK(LYH), NYH, Y, IFLAG)
...
C the call to the one-step core integrator DSTODE2.
C
...
C-----------------------------------------------------------------------
 ...
 290  CONTINUE
C-----------------------------------------------------------------------
C  CALL DSTODE2(NEQ,Y,YH,NYH,YH,EWT,SAVF,ACOR,WM,IWM,F,JAC,DPREPJ2,DSOLSY2)
C-----------------------------------------------------------------------
      CALL DSTODE2 (NEQ, Y, RWORK(LYH), NYH, RWORK(LYH), RWORK(LEWT),
     1   RWORK(LSAVF), RWORK(LACOR), RWORK(LWM), IWORK(LIWM),
     2   F, JAC, DPREPJ2, DSOLSY2)
      KGO = 1 - KFLAG
...
C-----------------------------------------------------------------------
 ...
 616  MSG = 'LSODE- Energy-  HMIN (=R1) .LT. 0.0  '
      ...
      GO TO 700
 617  CONTINUE
      MSG='LSODE- Energy-  RWORK length needed, LENRW (=I1), '//
     $    'exceeds LRW (=I2)'
      CALL XERRWD (MSG, 68, 17, 0, 2, LENRW, LRW, 0, 0.0, 0.0)
      GO TO 700
 618  CONTINUE
      MSG='LSODE- Energy-  IWORK length needed, LENIW (=I1), '//
     $    'exceeds LIW (=I2)'
...
C----------------------- END OF SUBROUTINE LSODE ----------------------
      ...
      SUBROUTINE DSRCOM2 (RSAV, ISAV, JOB)
C***BEGIN PROLOGUE  DSRCOM2
C***SUBSIDIARY
...
C  the COMMON block DLS002, which is used internally
...
C
C***FIRST EXECUTABLE STATEMENT  DSRCOM2
...
C
 ...
 120     ILS(I) = ISAV(I)
      RETURN
C----------------------- END OF SUBROUTINE DSRCOM2 ----------------------
      END SUBROUTINE DSRCOM2
      ...
      SUBROUTINE DINTDY2 (T, K, YH, NYH, DKY, IFLAG)
C***BEGIN PROLOGUE  DINTDY2
C***SUBSIDIARY
...
C***DESCRIPTION
C
C  DINTDY2 computes interpolated values of the K-th derivative of the
...
!$OMP THREADPRIVATE(/DLS002/)
C
C***FIRST EXECUTABLE STATEMENT  DINTDY2
      IFLAG = 0
...
C
 ...
 60     DKY(I) = R*DKY(I)
      RETURN
C
 80   MSG = 'DINTDY2-  K (=I1) illegal      '
      CALL XERRWD (MSG, 30, 52, 0, 1, K, 0, 0, 0.0, 0.0)
      IFLAG = -1
      RETURN
 90   MSG = 'DINTDY2-  T (=R1) illegal      '
      CALL XERRWD (MSG, 30, 52, 0, 0, 0, 0, 1, T, 0.0)
      MSG='      T not in interval TCUR - HU (= R1) to TCUR (=R2)      '
      CALL XERRWD (MSG, 60, 52, 0, 0, 0, 0, 2, TP, TN)
      IFLAG = -2
      RETURN
C----------------------- END OF SUBROUTINE DINTDY2 ----------------------
      END SUBROUTINE DINTDY2
      ...
      SUBROUTINE DPREPJ2 (NEQ, Y, YH, NYH, EWT, FTEM, SAVF, WM, IWM,
     1   F, JAC)
C***BEGIN PROLOGUE  DPREPJ2
C***SUBSIDIARY
...
C***DESCRIPTION
C
C  DPREPJ2 is called by DSTODE2 to compute and process the matrix
...
C
C  In addition to variables described in DSTODE2 and LSODE prologues,
C  communication with DPREPJ2 uses the following:
C  Y     = array containing predicted values on entry.
C  FTEM  = work array of length N (ACOR in DSTODE2).
...
!$OMP THREADPRIVATE(/DLS002/)
C
C***FIRST EXECUTABLE STATEMENT  DPREPJ2
      NJE = NJE + 1
      IERPJ = 0
      JCUR = 1
      HL0 = H*EL0
...
C Do LU decomposition of P. --------------------------------------------
      ...
      IF (IER .NE. 0) IERPJ = 1
      RETURN
C----------------------- END OF SUBROUTINE DPREPJ2 ----------------------
      END SUBROUTINE DPREPJ2
      ...
      SUBROUTINE DSOLSY2 (WM, IWM, X, TEM)
C***BEGIN PROLOGUE  DSOLSY2
C***SUBSIDIARY
...
C  Communication with DSOLSY2 uses the following variables:
...
!$OMP THREADPRIVATE(/DLS002/)
C
C***FIRST EXECUTABLE STATEMENT  DSOLSY2
      IERSL = 0
...
C
 400  ML = IWM(1)
      ...
      RETURN
C----------------------- END OF SUBROUTINE DSOLSY2 ----------------------
      END SUBROUTINE DSOLSY2
      ...
      SUBROUTINE DSTODE2 (NEQ, Y, YH, NYH, YH1, EWT, SAVF, ACOR,
     1   WM, IWM, F, JAC, PJAC, SLVS)
C***BEGIN PROLOGUE  DSTODE2
C***SUBSIDIARY
...
C***DESCRIPTION
C
C  DSTODE2 performs one step of the integration of an initial value
C  problem for a system of ordinary differential equations.
C  Note:  DSTODE2 is independent of the value of the iteration method
C  indicator MITER, when this is .ne. 0, and hence is independent
C  of the type of chord method used, or the Jacobian structure.
C  Communication with DSTODE2 is done with the following variables:
C
...
C**End
     ...
     1   ACOR(*), WM(*), IWM(*)
      COMMON /DLS002/ CONIT, CRATE, EL(13), ELCO(13,12),
...
!$OMP THREADPRIVATE(/DLS002/)
C
C***FIRST EXECUTABLE STATEMENT  DSTODE2
      KFLAG = 0
      TOLD = TN
      NCF = 0
      IERPJ = 0
...
C-----------------------------------------------------------------------
 ...
 720  HOLD = H
      JSTART = 1
      RETURN
C----------------------- END OF SUBROUTINE DSTODE2 ----------------------
      END SUBROUTINE DSTODE2





      END MODULE LSODE2_MOD
